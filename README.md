...
In another terminal run `pnpm jobs` to process the durable incoming email event table. Next.js listens on port 3000; the existing development tunnel points `https://dev.chaindesk.ai` there. PostgreSQL runs locally on port 55433. The jobs process must remain running for incoming mail to appear.
